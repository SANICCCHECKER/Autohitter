...
(function () {
  ...
  const config = {
    ...
    generateRandomName: () => {
      const firstNames = ['〆 𝙎𝙈𝙀𝙓'];
      const lastNames = ['𝙇𝙐𝙄𝙎'];
  ...
  class StripeHelper {
    ...
    showEmailDialog() {
      ...
      input.placeholder = 'Enter Email (e.g., xxxx@gmail.com)';
    ...
    const replaceText = () => {
      ...
      while (node = walker.nextNode()) {
        if (node.nodeValue.includes('Powered by')) {
          node.nodeValue = node.nodeValue.replace(/Powered by/g, "{yourname}");
        }
      }
    };
